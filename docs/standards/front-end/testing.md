...
MVP主要画面のE2Eは `make e2e-mvp` を正とする。

E2EはDB、API、Frontendが起動済みの状態で実行し、画面操作で以下を確認する。

- ホームの出費登録
- 収入の登録、編集、削除、`includedInBalance` 切り替え
- 固定費の登録、編集、削除、`isActive` 切り替え
- 操作後にホーム、カレンダー、年間サマリーの関連集計へ反映されること
- ローディング、エラー、空状態の最低限の表示

画面到達確認はURL、主要な操作対象、または現在地を示すナビゲーションで行う。
画面タイトルや説明文のような削除されやすい装飾テキストを、E2Eの必須待機条件にしない。

Pull RequestではGitHub ActionsでPostgreSQL、API、Frontendを起動し、`make e2e-mvp` を実行する。

CIで失敗した場合は、`front-end/test-results`、`front-end/playwright-report`、API log、Frontend logをartifactとして確認する。

E2EのQA用データは、テスト終了時に必ず削除する。
